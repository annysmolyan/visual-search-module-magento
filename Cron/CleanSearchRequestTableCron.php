...
/**
 * Class CleanSearchRequestTableCron
...
class CleanSearchRequestTableCron
{
    ...
    public function execute(): void
    {
        if (!$this->canRunCron()) {
            $this->logger->debug(
                'Cron Job belsmol_search_request_table_clean_cron: Module disabled or can not clean search request table by cron.'
            );
            return;
        }

        $rowsToKeep = $this->configStorage->getSearchRequestRowsSavedCount();
        $this->dbManager->cleanVisualSearchRequestTable($rowsToKeep);

        $this->logger->debug(
            'Cron Job belsmol_search_request_table_clean_cron: queue table has been cleaned.'
        );
    }
    ...
    private function canRunCron(): bool
    {
        return $this->configStorage->isModuleEnabled()
            && $this->configStorage->canCleanSearchRequestTableByCron();
    }
}
